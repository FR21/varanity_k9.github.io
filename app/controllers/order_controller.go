...
import (
	...
	"github.com/FR21/ap3_toko/app/models"
...
func (server *Server) Checkout(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		SetFlash(w, r, "error", "Proses checkout gagal")
		http.Redirect(w, r, "/carts"+order.ID, http.StatusSeeOther)
	}

	ClearCart(server.DB, cartID)

	SetFlash(w, r, "success", "Data order berhasil disimpan")
	http.Redirect(w, r, "/https://api.whatsapp.com/send?phone=628123456789", http.StatusSeeOther)
}
